globals [
  ...
  spacing-constant     ;; A constant that influences the calculations and behavior
  proposals            ;; Holds the list of proposals made by free turtles
]

turtles-own [
  pointer              ;; Pointer that stores the partner each turtle points to
  state                ;; Current state of the turtle (e.g., matched, free, waiting)
]

;; The setup procedure is used to initialize the environment and create turtles
to setup
  clear-all
  ;; Set the spacing constant based on the number of nodes
  set spacing-constant number-of-nodes + 1

  ;; Calculate the maximum number of links (edges) that can exist between turtles
  ...
  create-turtles number-of-nodes [
    ...
    set size max list 1 10 / sqrt number-of-nodes  ;; Scale the size of turtles based on the number of nodes
    setxy random-xcor random-ycor  ;; Place the turtles randomly in the world
    set pointer nobody  ;; Initially, turtles have no pointer (not matched)
  ]

  ;; Create links (edges) between turtles until the target number of links is reached
  while [ count links < min list number-of-links max-links ] [
    let a one-of turtles
    let b one-of other turtles with [not link-neighbor? a]  ;; Pick two turtles that aren't already linked
    if a != b [
      ask a [ create-link-with b ]  ;; Create a link between these two turtles
    ]
  ]
...
to go
  ...
  ask turtles [
    if pointer != nobody and [pointer] of pointer = self [  ;; If a turtle points to itself, it's "matched"
      set state "matched"
      set matched-count matched-count + 1
    ]
  ]

  ;; Waiting turtles: If a turtle is not yet assigned a pointer but has neighbors, it's in the "waiting" state
  ...
  ;; Free turtles: If a turtle has no pointer and is not yet waiting, it is considered "free"
  ask turtles with [state = ""] [
  ...
  ;; Single turtles: If a turtle is not matched but all its neighbors are already pointing to others, it's "single"
  ask turtles with [state = ""] [
  ...
  ;; Chaining turtles: If a turtle is pointing to another turtle that is not pointing back, it enters the "chaining" state
  ask turtles with [state = ""] [
  ...
  ask turtles with [state = "free"] [
    ...
    if target != nobody [
      set proposals lput (list self target) proposals  ;; Add the proposal to the list
    ]
  ]
  ...
  (foreach proposals [
    pair ->
      ...
      ask from-item [ set pointer to-item ]  ;; Turtle "from-item" points to "to-item"
  ])
  ...
  ask turtles with [state = "matched"] [
    let partner pointer
    if partner != nobody [
      ask link-with partner [ set color red ]  ;; Set the link color to red for matched turtles
    ]
  ]
...
;; This function calculates the VF(c) value based on the current counts of states
to-report vf
...
;; Perturbation procedure randomly changes pointers of some turtles to simulate disruptions
to perturbation
  let num-to-perturb floor (number-of-nodes * 0.1)  ;; 10% of turtles will be perturbed
  let targets n-of num-to-perturb turtles  ;; Choose the turtles to perturb

  ask targets [
    let non-neighbors other turtles with [not link-neighbor? myself and self != myself]
    if any? non-neighbors [
      set pointer one-of non-neighbors  ;; Assign a random non-neighbor as a new pointer
    ]
  ]
end
@#$#@#$#@
